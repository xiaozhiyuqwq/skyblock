...
tellraw @a[scores={login=0,language=1,level=1..5,sign_in=1}] {"rawtext":[{"text":"§fHello, welcome to join the game! §6welcome back! "}]}
...
tellraw @a[scores={login=0,language=0}] {"rawtext":[{"text":"§fCurrently running Rainyat SkyBlock Function v3.8 PRO"}]}
tellraw @a[scores={login=0,language=1}] {"rawtext":[{"text":"§f当前正在运行 雨逸阁空岛生存函数包 v3.8 PRO"}]}
...
scoreboard players add @a[scores={login=0}] login 1